...
interface RespostaLogin {
  ...
  export const login = async (usuario: usuario): Promise<RespostaLogin> => {
    try {
      const response = await axios.post<RespostaLogin>('http://192.168.0.108:3000/token', usuario);
      return response.data;
    } catch (erro: any) {

      if (erro.response.data) {
        throw new Error(erro.response.data.error); // propaga a mensagem para o componente
    }
...
export const register = async (usuario: NovoUsuario): Promise<RespostaApi> => {
  try {
    const response = await axios.post<RespostaApi>('http://192.168.0.108:3000/users', usuario);
